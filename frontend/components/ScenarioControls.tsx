...
export default function ScenarioControls({
  ...
  return (
    <div className="absolute top-20 left-6 z-10 bg-slate-900/95 backdrop-blur-sm rounded-lg shadow-xl border border-slate-700 w-80">
      {/* Header */}
      <div
        className="px-4 py-2.5 bg-slate-800 text-white border-b border-slate-700 rounded-t-lg flex items-center justify-between cursor-pointer hover:bg-slate-750"
        ...
        <div className="p-4 space-y-4 max-h-[calc(100vh-8rem)] overflow-y-auto">
          ...
          {isScenarioActive && metrics && (
            <div className="bg-slate-800/50 border border-slate-600 rounded-lg p-3 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div>
                  <div className="text-xs text-slate-400">Avg CSI</div>
          ...
          {(params.car !== 0 || params.transit !== 0 || params.trees !== 0) && (
            <button
              ...
              className="w-full py-1.5 px-4 text-sm text-slate-300 hover:text-white hover:bg-slate-800 rounded transition-colors border border-slate-700"
            >
              Reset to Current
            </button>
          )}
